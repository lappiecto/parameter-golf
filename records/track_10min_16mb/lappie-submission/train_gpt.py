"""
Lappie Parameter Golf submission — 10min / 16MB track.

Integrates the following techniques on top of the baseline train_gpt.py:
  ...
  - LN Scale: 1/sqrt(layer_idx+1) scaling on residual contributions
  - Cautious Muon: mask momentum that conflicts with gradient direction
...
class Hyperparameters:
    ...
    # Partial RoPE: only rotate the first rope_dims of each head_dim
    rope_dims = int(os.environ.get("ROPE_DIMS", 16))

    # N-gram embedding modules
...
def zeropower_via_newtonschulz5(G: Tensor, steps: int = 10, eps: float = 1e-7) -> Tensor:
    """Orthogonalise a 2D update matrix with a fast Newton-Schulz iteration."""
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        # Gated Attention: per-head sigmoid gate (init 0 -> 0.5 blend)
...
# ---------------------------------------------------------------------------
# Block with LN Scale + VRL passthrough
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# GPT Model with N-gram modules, VRL caching, orthogonal init
# ---------------------------------------------------------------------------
...
class GPT(nn.Module):
    def __init__(
        ...
        # Transformer blocks with layer_idx and rope_dims
...
def main() -> None:
    ...
    # Enable flash SDP where available (H100/A100), fall back to math SDP on older GPUs (T4/V100)
    ...
    # -------------------------------------------------------------------------
    # Optimizer groups — split by parameter role
    # -------------------------------------------------------------------------
    ...
    # N-gram module parameters
    ngram_embed_params: list[nn.Parameter] = []  # 2D embeddings -> embed_lr
    ngram_matrix_params: list[nn.Parameter] = []  # 2D projections -> matrix_lr (Muon)
    ngram_scalar_params: list[nn.Parameter] = []  # 1D gates/scales -> scalar_lr
    ...
    scalar_params.extend(ngram_scalar_params)

    # Add n-gram matrix params to the Muon group
    matrix_params.extend(ngram_matrix_params)

    token_lr = args.tied_embed_lr if args.tie_embeddings else args.embed_lr

    # Token embedding optimizer (Adam)
    tok_optim_params = [base_model.tok_emb.weight]
    tok_optim_params.extend(ngram_embed_params)
    optimizer_tok = torch.optim.Adam(
        [{"params": tok_optim_params, "lr": token_lr, "base_lr": token_lr}],
    ...
    swa_count = 0
    swa_start_step = int(args.swa_start_frac * args.iterations) if args.swa_enabled else args.iterations + 1
    ...
    def swa_update() -> None:
        ...
        if swa_state is None:
            ...
            swa_count += 1
            for k in swa_state:
                swa_state[k] += (sd[k].detach().float() - swa_state[k]) / swa_count

    def swa_apply() -> None:
        if swa_state is None:
            return
        for k, v in swa_state.items():
            dtype = base_model.state_dict()[k].dtype
            swa_state[k] = v.to(dtype=dtype)
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        """Dual-path warmdown: min(iteration-based, wallclock-based) scaling."""
    ...
    while True:
        ...
        # SWA: accumulate weight averages after swa_start_step
    ...
    # Apply SWA averaged weights if we collected any
    if args.swa_enabled and swa_count > 0:
        ...
        log0(f"swa_val_loss:{swa_val_loss:.4f} swa_val_bpb:{swa_val_bpb:.4f}")
